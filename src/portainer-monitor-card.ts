...
function getAttr(hass: HomeAssistant, entityId: string | undefined, attr: string): unknown {
  if (!entityId) return undefined;
...
// ---------------------------------------------------------------------------
// LitElement base — use global lit from HA
// ---------------------------------------------------------------------------

// HA ships lit as a global. We access it via window to avoid bundling it.
const LitBase = (window as unknown as { LitElement?: CustomElementConstructor }).LitElement;

function html(strings: TemplateStringsArray, ...values: unknown[]): unknown {
  const litHtml = (window as unknown as { html?: (...args: unknown[]) => unknown }).html;
  if (litHtml) return litHtml(strings, ...values);
  // Fallback: build raw string for environments where lit isn't loaded yet
  let result = "";
  strings.forEach((s, i) => { result += s + (values[i] ?? ""); });
  return result;
}

function css(strings: TemplateStringsArray, ...values: unknown[]): unknown {
  const litCss = (window as unknown as { css?: (...args: unknown[]) => unknown }).css;
  if (litCss) return litCss(strings, ...values);
  let result = "";
  strings.forEach((s, i) => { result += s + (values[i] ?? ""); });
  return result;
}
...
function definCards(): void {
  const { LitElement, html: litHtml, css: litCss, nothing } = window as unknown as {
    LitElement: typeof HTMLElement & {
      new(): HTMLElement;
  ...
  class PortainerStackCard extends LitElement {
    ...
    render() {
      if (!this.hass || !this._config) return litHtml``;
              ...
              ${containers.map((c) => {
                ...
                return litHtml`
                  ${cCpu !== undefined ? litHtml`
                    <div class="stat-col">
                      <div class="stat-label">CPU ${c.name?.split("-").pop() ?? c.name}</div>
                      <div class="stat-value">${cCpu !== undefined ? `${parseFloat(cCpu as string).toFixed(2)}%` : "—"}</div>
                    </div>
                  ` : nothing}
                  ${cMem !== undefined ? litHtml`
                    <div class="stat-col">
                      <div class="stat-label">MEM ${c.name?.split("-").pop() ?? c.name}</div>
                      <div class="stat-value">${cMem !== undefined ? `${parseFloat(cMem as string).toFixed(1)}%` : "—"}</div>
                    </div>
                  ` : nothing}
                  <div class="stat-col">
                    <div class="stat-label">${c.name?.split("-").pop() ?? c.name}</div>
  ...
  function portainerEntityFilter(hass: HomeAssistant) {
    ...
    render() {
      ...
      return litHtml`
        <div class="editor-row">
          ...
          <ha-textfield label="Title" .value=${cfg.title ?? ""} @change=${this._inputChanged("title")}></ha-textfield>
        </div>
        <div class="editor-row">
          <span class="editor-label">IP Address / Port (e.g. 192.168.0.4:8765)</span>
          <ha-textfield label="IP : Port" .value=${cfg.ip_address ?? ""} @change=${this._inputChanged("ip_address")}></ha-textfield>
        </div>
        <div class="editor-row">
          <span class="editor-label">Icon (MDI)</span>
          <ha-textfield label="Icon" .value=${cfg.icon ?? ""} @change=${this._inputChanged("icon")} placeholder="mdi:layers-outline"></ha-textfield>
        </div>
        ...
        <div class="editor-row">
          <ha-formfield label="Show start/stop controls">
            <ha-checkbox
              ...
              @change=${(ev: Event) => this._valueChanged("show_controls", (ev.target as HTMLInputElement).checked)}
            ></ha-checkbox>
          </ha-formfield>
        </div>
      `;
  ...
  class PortainerContainerCardEditor extends LitElement {
    ...
    render() {
      ...
      return litHtml`
        ...
        <div class="editor-row">
          <span class="editor-label">Icon (MDI)</span>
          <ha-textfield label="Icon" .value=${cfg.icon ?? ""} @change=${this._inputChanged("icon")} placeholder="mdi:docker"></ha-textfield>
        </div>
        ...
        <div class="editor-row">
          ...
          <ha-entity-picker .hass=${this.hass} .value=${cfg.status_entity ?? ""} .entityFilter=${bsFilter} include-domains='["binary_sensor"]' @value-changed=${this._entityChanged("status_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">CPU Usage (sensor)</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.cpu_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("cpu_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Memory Usage % (sensor)</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.memory_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("memory_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Container Switch</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.container_switch_entity ?? ""} .entityFilter=${swFilter} include-domains='["switch"]' @value-changed=${this._entityChanged("container_switch_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Restart Button</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.restart_button_entity ?? ""} .entityFilter=${btnFilter} include-domains='["button"]' @value-changed=${this._entityChanged("restart_button_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Pause Button</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.pause_button_entity ?? ""} .entityFilter=${btnFilter} include-domains='["button"]' @value-changed=${this._entityChanged("pause_button_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Resume Button</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.resume_button_entity ?? ""} .entityFilter=${btnFilter} include-domains='["button"]' @value-changed=${this._entityChanged("resume_button_entity")}></ha-entity-picker>
        </div>
      `;
    }
  }
  ...
  class PortainerEndpointCardEditor extends LitElement {
    ...
    render() {
      ...
      return litHtml`
        ...
        <div class="editor-row">
          <span class="editor-label">Icon (MDI)</span>
          <ha-textfield label="Icon" .value=${cfg.icon ?? ""} @change=${this._inputChanged("icon")} placeholder="mdi:server"></ha-textfield>
        </div>
        ...
        <div class="editor-row">
          ...
          <ha-entity-picker .hass=${this.hass} .value=${cfg.containers_count_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("containers_count_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Running</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.containers_running_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("containers_running_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Stopped</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.containers_stopped_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("containers_stopped_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Paused</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.containers_paused_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("containers_paused_entity")}></ha-entity-picker>
        </div>

        <div class="editor-section">System Info</div>
        <div class="editor-row">
          <span class="editor-label">Docker Version</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.docker_version_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("docker_version_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Operating System</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.os_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("os_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Total RAM</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.memory_total_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("memory_total_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">CPU Count</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.cpu_total_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("cpu_total_entity")}></ha-entity-picker>
        </div>

        <div class="editor-section">Disk Usage</div>
        <div class="editor-row">
          <span class="editor-label">Image Disk Total</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.image_disk_total_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("image_disk_total_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Image Disk Reclaimable</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.image_disk_reclaimable_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("image_disk_reclaimable_entity")}></ha-entity-picker>
        </div>
        <div class="editor-row">
          <span class="editor-label">Container Disk Total</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.container_disk_total_entity ?? ""} .entityFilter=${snFilter} include-domains='["sensor"]' @value-changed=${this._entityChanged("container_disk_total_entity")}></ha-entity-picker>
        </div>

        <div class="editor-section">Actions</div>
        <div class="editor-row">
          <span class="editor-label">Prune Images Button</span>
          <ha-entity-picker .hass=${this.hass} .value=${cfg.prune_images_button_entity ?? ""} .entityFilter=${btnFilter} include-domains='["button"]' @value-changed=${this._entityChanged("prune_images_button_entity")}></ha-entity-picker>
        </div>
      `;
    }
  }
